...
CHANNELS = ["@Armylooterz2", "@Genuines_airdrops", "@AirdropNext_Level", "@Airdropo_o"] #add channels to be checked here in the format - ["Channel 1", "Channel 2"] 
   ...
   try:
    ...
    if call.data == 'check':
        if ch == True:
            ...
            markup.add(telebot.types.InlineKeyboardButton(
                text='Joined', callback_data='check'))
            msg_start = "* You Didn't Joined Channels :-\n\n🔎 Join Our All Channels:-\n *"
...
💰Balance : *`{}`* {}*'
    ...
    if message.text == "➖ Withdraw":
        ...
        if bal >= Mini_Withdraw:
            ...
            bot.send_message(
                user_id, f"_our balance low you should have at least {Mini_Withdraw} {TOKEN} to Withdraw_", parse_mode="Markdown")
            return
   except:
        bot.send_message(message.chat.id, "This command having error pls wait for ficing the glitch by admin")
        bot.send_message(OWNER_ID, "Your bot got an error fix it fast!\n Error on command: "+message.text)
...
def trx_address(message):
   try:
    ...
    if len(message.text) == 34:
        ...
        bot.send_message(message.chat.id, "*Your EFM wallet set to " +
...
def amo_with(message):
   try:
    ...
    if msg.isdigit() == False:
        bot.send_message(
            user_id, "_Invaild value. Enter only numeric value. Try again_", parse_mode="Markdown")
        return
